...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    user: process.env.SMTP_USER || "azureclinic26@gmail.com",
    pass: process.env.SMTP_PASS || "smbk ziro qmop lmxs"
  }
});
...
async function sendVerificationEmail(email: string, name: string, token: string) {
  ...
  const mailOptions = {
    from: process.env.EMAIL_FROM || '"Azure Dental Clinic" <azureclinic26@gmail.com>',
    to: email,
